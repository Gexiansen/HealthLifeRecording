...
test("Service Worker 缓存精简后的完整应用外壳", async () => {
  ...
  assert.match(sw, /healthlife-shell-v30/);
  for (const file of [
  ...
  assert.match(app, /register\("\.\/sw\.js\?v=30"\)/);
  for (const file of [
    ...
    assert.match(app, new RegExp(`\\./${file.replace(".", "\\.")}\\?v=30`));
    assert.match(sw, new RegExp(`\\./${file.replace(".", "\\.")}\\?v=30`));
  }
  assert.match(sw, /\.\/health-stage\.js\?v=30/);
});
