...
- schema v6 使用独立存储键；有效 v5 数据自动迁移且保留旧键，v1／v2／v3／v4 不读取。
...
2. 当前页只展示一个动作、当前组、目标值、准备姿势和三条关键提示。
3. 常见错误和停止条件默认收起，需要时展开。
4. 完成本组后先保存草稿，再进入休息倒计时；可以跳过休息。
5. 支持跳过当前动作、提前结束并进入总结，以及退出后继续。
6. 结束页展示实际动作、组次、次数／楼层和负重；用户确认后才生成正式运动记录。
...
- 导出 schema v6 完整数据，并保留已完成引导式训练的动作快照。
...
| `docs/model.js` | schema v6、v5 迁移以及所有输入和跨对象校验 |
| `docs/storage.js` | v6 本地加载、训练草稿和写入失败保护 |
