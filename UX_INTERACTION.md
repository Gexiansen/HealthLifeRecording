...
- 当前 v6 数据损坏时保留原始内容，停止写入，并提供原始内容下载入口。
- schema v7 使用独立存储键；有效 v6 数据自动迁移，v5 可链式迁移且旧键保留，v1／v2／v3／v4 不读取。
...
计划卡提供“开始训练”入口，并根据力量 A、力量 B 或步行计划突出推荐模板。引导训练是全屏二级流程，不新增一级导航。

1. 选择动作适应、全身力量 A、全身力量 B 或初级爬楼梯模板。
2. 当前页只展示一个动作、当前组、目标值、准备姿势和三条关键提示，并显示实际执行的同动作最近三次记录。
3. 常见错误和停止条件默认收起，需要时展开。
4. 最近两次同动作都完整完成、用力适中且无不适时，才给出保守加量建议；建议不会自动修改本组目标或负重。
5. 首组开始前可以选择同动作模式的替代动作；替代选择写入草稿，历史对比始终按实际动作匹配。
6. 完成本组后先保存草稿，再进入休息倒计时；可以跳过休息。
7. 支持跳过当前动作、提前结束并进入总结，以及退出后继续。
8. 结束页展示实际动作、组次、次数／楼层／分钟和负重，并可按动作记录不适部位和 1～3 级程度；用户确认后才生成正式运动记录。

训练草稿、计划状态和正式运动记录相互独立。计划打钩不会生成运动记录；训练过程中退出不会把未完成计划伪装为完成。
...
- 导出 schema v7 完整数据，并保留已完成引导式训练的计划／实际动作、不适反馈和组次快照。
...
| `docs/app.js` | 页面状态、事件、渲染、引导训练流程、保存、删除、备份和反馈 |
| `docs/interaction.js` | 日期语境、默认餐次、快捷饮水、筛选和恢复文案 |
| `docs/planning.js` | 工作日类型、训练模板和当天建议 |
| `docs/guided-workout.js` | 内置动作、训练模板、逐组推进和训练快照 |
| `docs/calendar.js` | 周／月日历、完成状态和连续记录 |
| `docs/stats.js` | 7 天／30 天趋势与上一周期比较 |
| `docs/model.js` | schema v7、v6／v5 迁移以及所有输入和跨对象校验 |
| `docs/storage.js` | v7 本地加载、训练草稿迁移和写入失败保护 |
| `docs/training-insights.js` | 同动作历史、保守渐进建议和不适趋势汇总 |
