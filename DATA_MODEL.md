...
schema v10 使用 `healthlife:data:v10` 独立存储键。当前应用只读取和写入 v10，不读取、不迁移 v1 至 v9 数据。完整备份使用 `backupVersion: 10`，只接受 v10 备份。
...
- 体重以整数克保存，运动时长使用整数分钟；饮食原文不拆分为食物明细。
...
餐食包含餐次和 `content` 原文。`content` 长度为 1～2000 个字符，去除首尾空白后不得为空。用户可以写食物名称、克数、个数、包装份量、外卖菜名或无法精确量化的描述；应用只保存原文，不在记录时计算热量或宏量营养素。
...
- 完整备份保留每周模板和四类正式记录，用于完整替换恢复；当前只接受 v10 备份。
- 分析 JSON 按自然日输出体重、睡眠、运动和餐食原文，供后续健康习惯分析；不把文字内容伪装成营养测量值。
